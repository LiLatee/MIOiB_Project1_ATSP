...
plot <- ggplot(temp, aes(x=timeOfRunningInMs, y=diff, colour=algName)) +
  #geom_line(size=1) +
  geom_point() +
...
```{r zad2_point4}
temp <- group_by(results, algName, setName)
temp <- summarize(temp, meanNumberOfSteps=mean(numberOfSteps), std=sd(numberOfSteps), nOfCities=mean(nOfCities))
...
```{r zad2_point5}
temp <- group_by(results, algName, setName)
temp <- summarize(temp, meanNumberOfCheckedResults=mean(numberOfCheckedResults), std=sd(numberOfCheckedResults), nOfCities=mean(nOfCities))
...
plot <- ggplot(temp, aes(x=initialResult, y=finalResult, group=algName, colour=algName)) +
...
temp$meanResult = meanResult
 ...
 plot <- ggplot(temp, aes(x=iterationNumber)) +
