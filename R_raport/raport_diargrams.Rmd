...
plot <- ggplot(gp, aes(x=reorder(setName, nOfCities), y=diff, std=std, group=algName, fill=factor(algName))) +
  ...
  print(plot)
  cairo_pdf(filename, family="DejaVu Sans") # polskie znaki zapisuje poprawnie
```
...
## Zadanie 2 - punkt 3
```{r zad2_point3_all}
# gp <- filter(results, iterationNumber==0 | iterationNumber==9)
# gp <- select(results, algName, setName, initialValue, finalValue)
# gp <- group_by(results, algName, setName)
# gp <- summarize(gp, initialValue=max(result), finalValue=min(result), nOfCities=mean(nOfCities))
# knitr::kable(gp)
# 
# plot <- ggplot(gp, aes(x=initialValue, y=finalValue, group=algName)) +
#   facet_wrap(~setName) +
#   geom_point()
# print(plot)

# pd <- position_dodge(0.3)
# plot <- ggplot(gp, aes(x=iterationNumber, y=timeOfRunningInMs, group=algName, colour=algName)) +
#   #geom_line(aes(linetype=algName)) +
#   geom_point(size=3, shape=21, fill="white", pd=position_dodge(0.1), position=pd) +
#   geom_text(aes(label=sprintf("%0.1f", round(timeOfRunningInMs, digits=2))), position=position_dodge(width=0.9), vjust=-0.25, size=3) +
#   labs(x="Instancje", y="Czas działania [ms]") +
